...
*   In the initial state of the application, we provide you a custom hook, **useFetchHelloFreshBox**, that simulates data fetching from a remote server. It is a simple timeout function that returns static JSON data.
*   As this is a **full-stack challenge**, we expect you to replace the static data consumed by the **useFetchHelloFreshBox** with data coming from the **recipes-service** you will build.
...
* A database should be used to store the recipe information. DB username: recipe_db_user, DB password: hellofresh. The proposed tables' schemas are:
...
* Populate the DB with data defined in `/src/data/initial-data.sql`
